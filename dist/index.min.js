...
| --
| \\?.
`.replace(/\s+/g,""),"gsu");function st(e,t,r,s){return e.slice(0,t)+s+e.slice(t+r.length)}var at=new RegExp(String.raw`(?<noncapturingStart>${nt})|(?<capturingStart>\((?:\?<[^>]+>)?)|\\?.`,"gsu");function ot(e,t){if(!/\(\?>/.test(e))return e;let r="(?>",s=`(?:(?=(${t?.useEmulationGroups?rt:""}`,n=[0],o=0,a=0,i=NaN,c;do{c=!1;let u=0,p=0,h=!1,C;for(at.lastIndex=Number.isNaN(i)?0:i+s.length;C=at.exec(e);){let{0:d,index:w,groups:{capturingStart:T,noncapturingStart:_}}=C;if(d==="[")u++;else if(u)d==="]"&&u--;else if(d===r&&!h)i=w,h=!0;else if(h&&_)p++;else if(T)h?p++:(o++,n.push(o+a));else if(d===")"&&h){if(!p){a++,e=`${e.slice(0,i)}${s}${e.slice(i+r.length,w)}))<$$${a+o}>)${e.slice(w+1)}`,c=!0;break}p--}}}while(c);return e=M(e,String.raw`\\(?<backrefNum>[1-9]\d*)|<\$\$(?<wrappedBackrefNum>\d+)>`,({0:u,groups:{backrefNum:p,wrappedBackrefNum:h}})=>{if(p){let C=+p;if(C>n.length-1)throw new Error(`Backref "${u}" greater than number of captures`);return`\\${n[C]}`}return`\\${h}`},b.DEFAULT),e}var it=String.raw`(?:[?*+]|\{\d+(?:,\d*)?\})`,we=new RegExp(String.raw`
\\(?: \d+
  | c[A-Za-z]
  | [gk]<[^>]+>
  | [pPu]\{[^\}]+\}

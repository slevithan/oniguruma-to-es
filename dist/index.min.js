...
| --
| \\?.
`.replace(/\s+/g,""),"gsu");function Nt(e,{regexContext:t=_.DEFAULT,charClassContext:r=b.DEFAULT,charClassDepth:s=0,lastPos:n=0}={}){St.lastIndex=n;let a;for(;a=St.exec(e);){let{0:o,groups:{groupN:i,enclosedPU:u,enclosedQ:c,intervalQ:l,incompleteT:g}}=a;o==="["?(s++,t=_.CHAR_CLASS,r=b.DEFAULT):o==="]"&&t===_.CHAR_CLASS?(s&&s--,s||(t=_.DEFAULT),r=b.DEFAULT):t===_.CHAR_CLASS?g?r=b.INVALID_INCOMPLETE_TOKEN:o==="-"?r=b.RANGE:u?r=o[1]==="u"?b.ENCLOSED_U:b.ENCLOSED_P:c?r=b.ENCLOSED_Q:(o==="}"&&en.has(r)||r===b.INVALID_INCOMPLETE_TOKEN||r===b.RANGE)&&(r=b.DEFAULT):g?t=_.INVALID_INCOMPLETE_TOKEN:i?t=_.GROUP_NAME:u?t=o[1]==="u"?_.ENCLOSED_U:_.ENCLOSED_P:l?t=_.INTERVAL_QUANTIFIER:(o===">"&&t===_.GROUP_NAME||o==="}"&&(t===_.INTERVAL_QUANTIFIER||Yr.has(t))||t===_.INVALID_INCOMPLETE_TOKEN)&&(t=_.DEFAULT)}return{regexContext:t,charClassContext:r,charClassDepth:s,lastPos:e.length}}function sn(e){let t=0;return Pe(e,Ue,()=>t++,y.DEFAULT),t}function le(e,t,r,s){return e.slice(0,t)+s+e.slice(t+r.length)}var an=["Basic_Emoji","Emoji_Keycap_Sequence","RGI_Emoji_Modifier_Sequence","RGI_Emoji_Flag_Sequence","RGI_Emoji_Tag_Sequence","RGI_Emoji_ZWJ_Sequence","RGI_Emoji"].join("|"),us=new RegExp(String.raw`
\\(?: c[A-Za-z]
  | p\{(?<pStrProp>${an})\}
  | [pP]\{[^\}]+\}
  | (?<qStrProp>q)
...
| &&
| .
`.replace(/\s+/g,""),"gsu");function on(e,t,r,s){let n={raw:[]},a=[],o;return e.raw.forEach((i,u)=>{let c=r(i,{...o,lastPos:0},s);if(n.raw.push(c.transformed),o=c.runningContext,u<e.raw.length-1){let l=t[u];if(l instanceof Le){let g=r(l,{...o,lastPos:0},s);a.push(Zr(g.transformed)),o=g.runningContext}else a.push(l)}}),{template:n,substitutions:a}}var un=new RegExp(String.raw`
${Oe}
| \(\?<
| (?<backrefNum>\\[1-9]\d*)
| \\?.
`.replace(/\s+/g,""),"gsu");function cn(e,t){e=String(e);let r="",s="";for(let{0:n,groups:{backrefNum:a}}of e.matchAll(un)){r+=n,t=Nt(r,t);let{regexContext:o}=t;if(o===_.DEFAULT)if(n==="(")s+="(?:";else{if(a)throw new Error(`Invalid decimal escape "${n}" with implicit flag n; replace with named backreference`);s+=n}else s+=n}return{transformed:s,runningContext:t}}var kt=/^\s$/,ln=/^\\[\s#]$/,Ie=/^[ \t]$/,pn=/^\\[ \t]$/,fn=new RegExp(String.raw`
\\(?: [gk]<
  | [pPu]\{
  | c[A-Za-z]
  | u[A-Fa-f\d]{4}
...
| --
| \\?.
`.replace(/\s+/g,""),"gsu");function gn(e,t,r){e=String(e);let s=!1,n=!1,a=!1,o="",i="",u="",c="",l=!1,g=(p,{prefix:h=!0,postfix:m=!1}={})=>(p=(l&&h?"(?:)":"")+p+(m?"(?:)":""),l=!1,p);for(let{0:p,index:h}of e.matchAll(fn)){if(a){p===`
`&&(a=!1,l=!0);continue}if(s){if(kt.test(p))continue;s=!1,l=!0}else if(n){if(Ie.test(p))continue;n=!1}o+=p,t=Nt(o,t);let{regexContext:m,charClassContext:w}=t;if(p==="-"&&m===_.CHAR_CLASS&&c===b.RANGE&&(r.flags.includes("v")||r.unicodeSetsPlugin))throw new Error("Invalid unescaped hyphen as the end value for a range");if(m===_.DEFAULT&&/^(?:[?*+]|\?\?)$/.test(p)||m===_.INTERVAL_QUANTIFIER&&p==="{")i+=g(p,{prefix:!1,postfix:u==="("&&p==="?"});else if(m===_.DEFAULT)kt.test(p)?s=!0:p.startsWith("#")?a=!0:ln.test(p)?i+=g(p[1],{prefix:!1}):i+=g(p);else if(m===_.CHAR_CLASS&&p!=="["&&p!=="[^")if(Ie.test(p)&&(w===b.DEFAULT||w===b.ENCLOSED_Q||w===b.RANGE))n=!0;else{if(w===b.INVALID_INCOMPLETE_TOKEN)throw new Error(`Invalid incomplete token in character class: "${p}"`);if(pn.test(p)&&(w===b.DEFAULT||w===b.ENCLOSED_Q))i+=g(p[1],{prefix:!1});else if(w===b.DEFAULT){let k=e[h+1]??"",S=nn(p);(Ie.test(k)||p==="^")&&(S=rn(S)),i+=g(S)}else i+=g(p)}else i+=g(p);s||n||a||(u=p,c=w)}return{transformed:i,runningContext:t}}function hn(e){let t=String.raw`\(\?:\)`;e=Q(e,`(?:${t}){2,}`,"(?:)",y.DEFAULT);let r=De.replace(/\$/g,"\\$");return e=Q(e,String.raw`(?:${t}(?=[)|.[$\\]|\((?!DEFINE)|$)|(?<=[()|.\]^>]|\\[bBdDfnrsStvwW]|\(\?(?:[:=!]|<[=!])|^)${t}(?![?*+{]))(?!${r})`,"",y.DEFAULT),e}var _t=new RegExp(String.raw`(?<noncapturingStart>${Oe})|(?<capturingStart>\((?:\?<[^>]+>)?)|\\?.`,"gsu");function dn(e,t){if(!/\(\?>/.test(e))return e;let r="(?>",s=`(?:(?=(${t.useEmulationGroups?De:""}`,n=[0],a=0,o=0,i=NaN,u;do{u=!1;let c=0,l=0,g=!1,p;for(_t.lastIndex=Number.isNaN(i)?0:i+s.length;p=_t.exec(e);){let{0:h,index:m,groups:{capturingStart:w,noncapturingStart:k}}=p;if(h==="[")c++;else if(c)h==="]"&&c--;else if(h===r&&!g)i=m,g=!0;else if(g&&k)l++;else if(w)g?l++:(a++,n.push(a+o));else if(h===")"&&g){if(!l){o++,e=`${e.slice(0,i)}${s}${e.slice(i+r.length,m)}))<$$${o+a}>)${e.slice(m+1)}`,u=!0;break}l--}}}while(u);return e=Q(e,String.raw`\\(?<backrefNum>[1-9]\d*)|<\$\$(?<wrappedBackrefNum>\d+)>`,({0:c,groups:{backrefNum:l,wrappedBackrefNum:g}})=>{if(l){let p=+l;if(p>n.length-1)throw new Error(`Backref "${c}" greater than number of captures`);return`\\${n[p]}`}return`\\${g}`},y.DEFAULT),e}var It=String.raw`(?:[?*+]|\{\d+(?:,\d*)?\})`,ye=new RegExp(String.raw`
\\(?: \d+
  | c[A-Za-z]
  | [gk]<[^>]+>
  | [pPu]\{[^\}]+\}
...
| \\?.
`.replace(/\s+/g,""),"gsu");function Cn(e){if(!new RegExp(`${It}\\+`).test(e))return e;let t=[],r=null,s=null,n="",a=0,o;for(ye.lastIndex=0;o=ye.exec(e);){let{0:i,index:u,groups:{qBase:c,qMod:l,invalidQ:g}}=o;if(i==="[")a||(s=u),a++;else if(i==="]")a?a--:s=null;else if(!a)if(l==="+"&&n&&!n.startsWith("(")){if(g)throw new Error(`Invalid quantifier "${i}"`);let p=-1;if(/^\{\d+\}$/.test(c))e=le(e,u+c.length,l,"");else{if(n===")"||n==="]"){let h=n===")"?r:s;if(h===null)throw new Error(`Invalid unmatched "${n}"`);e=`${e.slice(0,h)}(?>${e.slice(h,u)}${c})${e.slice(u+i.length)}`}else e=`${e.slice(0,u-n.length)}(?>${n}${c})${e.slice(u+i.length)}`;p+=4}ye.lastIndex+=p}else i[0]==="("?t.push(u):i===")"&&(r=t.length?t.pop():null);n=i}return e}function mn(e,t){let r=yt(e,{includeContents:!0}),s=wn(e,r,t.useEmulationGroups);return An(s,r)}var En=String.raw`\\g<(?<subroutineName>[^>&]+)>`,re=new RegExp(String.raw`
${En}
| (?<capturingStart>${Ue})
| \\(?<backrefNum>[1-9]\d*)
| \\k<(?<backrefName>[^>]+)>
| \\?.
`.replace(/\s+/g,""),"gsu");function wn(e,t,r){if(!/\\g</.test(e))return e;let s=Jr(e,"\\\\(?:[1-9]|k<[^>]+>)",y.DEFAULT),n=s?`(${r?De:""}`:"(?:",a=new Map,o=[],i=[0],u=0,c=0,l=0,g=0,p=0,h=e,m;for(re.lastIndex=0;m=re.exec(h);){let{0:w,index:k,groups:{subroutineName:S,capturingStart:fe,backrefNum:X,backrefName:ne}}=m;if(w==="[")p++;else if(p)w==="]"&&p--;else if(S){if(!t.has(S))throw new Error(`Invalid named capture referenced by subroutine ${w}`);if(a.has(S))throw new Error(`Subroutine ${w} followed a recursive reference`);let F=t.get(S).contents,B=`${n}${F})`;s&&(l=0,c++),a.set(S,{unclosedGroupCount:Sn(B)}),o.push(S),h=le(h,k,w,B),re.lastIndex-=w.length-n.length}else if(fe)a.size?(s&&(l++,c++),w!=="("&&(h=le(h,k,w,n),re.lastIndex-=w.length-n.length)):s&&(i.push(xe(i)+1+c-g),g=c,u++);else if((X||ne)&&a.size){let F=X?+X:t.get(ne)?.groupNum,B=!1;for(let R of o){let V=t.get(R);if(F>=V.groupNum&&F<=V.groupNum+V.numCaptures){B=!0;break}}if(B){let R=t.get(xe(o)),V=u+c-l,J=`\\k<$$b${F}s${V}r${R.groupNum}c${R.numCaptures}>`;h=le(h,k,w,J),re.lastIndex+=J.length-w.length}}else if(w===")"&&a.size){let F=a.get(xe(o));F.unclosedGroupCount--,F.unclosedGroupCount||a.delete(o.pop())}}return s&&(h=Q(h,String.raw`\\(?:(?<bNum>[1-9]\d*)|k<\$\$b(?<bNumSub>\d+)s(?<subNum>\d+)r(?<refNum>\d+)c(?<refCaps>\d+)>)`,({0:w,groups:{bNum:k,bNumSub:S,subNum:fe,refNum:X,refCaps:ne}})=>{if(k){let J=+k;if(J>i.length-1)throw new Error(`Backref "${w}" greater than number of captures`);return`\\${i[J]}`}let F=+S,B=+fe,R=+X,V=+ne;return F<R||F>R+V?`\\${i[F]}`:`\\${B-R+F}`},y.DEFAULT)),h}var Fe=new RegExp(String.raw`${Re}|\(\?:\)|(?<invalid>\\?.)`,"gsu");function An(e,t){let r=Te(e,String.raw`\(\?\(DEFINE\)`,0,y.DEFAULT);if(!r)return e;let s=bt(e,r);if(s.afterPos<e.length)throw new Error("DEFINE group allowed only at the end of a regex");if(s.afterPos>e.length)throw new Error("DEFINE group is unclosed");let n;for(Fe.lastIndex=0;n=Fe.exec(s.contents);){let{captureName:a,invalid:o}=n.groups;if(a){let i=bt(s.contents,n),u;if(!t.get(a).isUnique)u=a;else{let c=yt(i.contents);for(let l of c.keys())if(!t.get(l).isUnique){u=l;break}}if(u)throw new Error(`Duplicate group name "${u}" within DEFINE`);Fe.lastIndex=i.afterPos}else if(o)throw new Error("DEFINE group includes unsupported syntax at top level")}return e.slice(0,r.index)}function Sn(e){let t=0;return Pe(e,"\\(",()=>t++,y.DEFAULT),t}function kn(e,t){let r=0,s=0,n;for(;n=Te(e,Ue,s,y.DEFAULT);){let{0:a,index:o,groups:{captureName:i}}=n;if(r++,i===t)break;s=o+a.length}return r}function bt(e,t){let r=t.index+t[0].length,s=$t(e,r),n=r+s.length+1;return{contents:s,afterPos:n}}function yt(e,{includeContents:t}={}){let r=new Map;return Pe(e,Re,({0:s,index:n,groups:{captureName:a}})=>{if(r.has(a))r.get(a).isUnique=!1;else{let o={isUnique:!0};if(t){let i=$t(e,n+s.length);Object.assign(o,{contents:i,groupNum:kn(e,a),numCaptures:sn(i)})}r.set(a,o)}},y.DEFAULT),r}function xe(e){return e[e.length-1]}var _n="&!#%,:;<=>@`~",bn=new RegExp(String.raw`
\[\^?-?
| --?\]
| (?<dp>[${ve}])\k<dp>
| --

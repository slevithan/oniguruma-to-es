...
Run times: These are included in the results, providing a rough performance comparison between
Oniguruma and regexes generated by this library. The reported run times currently include only the
first search; not the time to find all results. For Oniguruma, it includes the time to load the
WASM module (in other words, it's the time to run the regex in Oniguruma *from JS*). For the
library, it includes the time to transpile the pattern, in addition to the time spent running the
search using the resulting JS `RegExp`.
*/

...
function getArgs(actual, expected) {
  ...
  return {
    ...
    maxRecursionDepth: expected.maxRecursionDepth ?? 5,
...
function matchWithAllTargets({pattern, flags, accuracy, maxRecursionDepth, rules, strings, targets}, {exact, negate}) {
  for (const target of targets) {
    const re = toRegExp(pattern, {accuracy, flags, maxRecursionDepth, rules, target});
